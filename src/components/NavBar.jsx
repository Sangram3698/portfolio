...
const NavBar = () => {
  ...
  return (
    <div className="flex justify-between items-center w-full h-20 px-4 bg-black text-white fixed">
      <div>
        <h1 className="text-5xl font-signature ml-2">SangramVerse</h1>
      </div>
      ...
      <ul className="hidden md:flex">
        {links.map(({ id, link }) => (
          ...
          ><Link to={link} smooth duration={450}>{link}</Link></li>
        ))}
      </ul>
        ...
        <ul className="flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-gradient-to-b from-black to-gray-800 text-white">
          {links.map(({ id, link }) => (
            <li
              ...
              onClick={() => setNav(!nav)}
            ><Link onClick={()=>setNav(!nav)} to={link} smooth duration={450}>{link}</Link></li>
          ))}
        </ul>
      )}
    </div>
